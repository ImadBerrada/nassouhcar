...
import BookingModal from '@/app/en/components/BookingModal'
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 to-slate-100">
        <div className="text-center">
          ...
          <p className="text-slate-600 text-lg">Loading our fleet in Tangier…</p>
        </div>
      </div>
    )
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 to-slate-100">
        <div className="text-center">
          ...
          <button onClick={fetchCars} className="px-6 py-2 bg-yellow-500 text-white rounded-lg hover:bg-yellow-600 transition-colors">
            Retry
          </button>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100">
      ...
      <div className="relative bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 py-16 overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <h1 className="text-6xl md:text-7xl font-black text-white mb-6 leading-tight tracking-tight">Car Rental Tangier – Economy Fleet</h1>
          <p className="text-xl md:text-2xl text-gray-300 max-w-4xl mx-auto leading-relaxed">
            Discover our economy vehicle fleet for car rental in Tangier. From budget cars to family vehicles, find the perfect car for your trip in Morocco with Nassoh Car.
          </p>
          <div className="mt-8 flex flex-wrap justify-center gap-4 text-amber-400">
            <span className="bg-white/10 px-4 py-2 rounded-full">✓ Tangier Ibn Battouta Airport</span>
            <span className="bg-white/10 px-4 py-2 rounded-full">✓ Tangier Med Port</span>
            <span className="bg-white/10 px-4 py-2 rounded-full">✓ Tangier City Center</span>
          </div>
        </div>
      </div>

      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
        <Breadcrumb items={[{ label: 'Our Fleet', href: '/en/fleet' }]} className="text-slate-600" />
      </div>

      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="mb-8">
          <h2 className="text-2xl font-bold text-slate-900 mb-2">Available Vehicles in Tangier ({filteredCars.length})</h2>
          <p className="text-slate-600">{searchQuery ? `Results for "${searchQuery}"` : 'All vehicles available for rental in Tangier'}</p>
        </div>
        ...
        {filteredCars.length === 0 && (
          <div className="text-center py-16">
            ...
            <p className="text-slate-600 mb-6">{searchQuery ? 'Try adjusting your search terms' : 'No vehicles are currently available'}</p>
            {searchQuery && (
              <button onClick={() => setSearchQuery('')} className="px-6 py-3 bg-yellow-500 text-white rounded-xl hover:bg-yellow-600 transition-colors">
                Clear Search
              </button>
            )}
          </div>
              ...
              <div key={car.id} className={`bg-white rounded-2xl shadow-xl overflow-hidden transform transition-all duration-500 group hover:scale-[1.03] hover:shadow-2xl hover:-translate-y-2 border border-gray-100 ${viewMode === 'list' ? 'flex flex-col sm:flex-row' : ''}`}>
                <div className={`relative overflow-hidden ${viewMode === 'list' ? 'w-80 flex-shrink-0' : 'h-72'}`}>
                  ...
                  <div className="absolute top-4 left-4 z-20">
                    <span className="bg-gradient-to-r from-yellow-400 to-yellow-500 text-white px-4 py-2 rounded-full text-sm font-semibold shadow-lg">{car.category}</span>
                  </div>
                  <div className="absolute top-4 right-4 flex flex-col gap-2 z-20">
                    ...
                    <div className={`px-3 py-2 rounded-full text-xs font-semibold shadow-lg ${(car.availableQuantity || car.quantity) > 0 ? 'bg-green-500 text-white' : 'bg-red-500 text-white'}`}>
                      {(car.availableQuantity || car.quantity) > 0 ? `${car.availableQuantity || car.quantity} Available` : 'Not Available'}
                    </div>
                  </div>
                </div>
                <div className="p-6 flex flex-col flex-grow">
                  ...
                  <div className="flex items-center justify-between pt-6 border-t border-gray-100 mt-auto">
                    <div className="flex flex-col">
                      <span className="text-3xl font-bold text-gray-900">from €{car.pricePerDay.toFixed(2)}</span>
                      <span className="text-sm text-gray-500 font-medium">per day</span>
                    </div>
                    <button onClick={() => handleBookNow(car)} disabled={(car.availableQuantity || car.quantity) === 0} className={`px-6 py-2.5 rounded-lg font-medium transition-colors duration-200 text-sm ${(car.availableQuantity || car.quantity) > 0 ? 'bg-yellow-500 text-white hover:bg-yellow-600 focus:ring-2 focus:ring-yellow-500 focus:ring-offset-2' : 'bg-gray-300 text-gray-500 cursor-not-allowed'}`}>
                      {(car.availableQuantity || car.quantity) > 0 ? 'Book Now' : 'Not Available'}
                    </button>
                  </div>
                </div>
              </div>
      ...
      {isBookingModalOpen && selectedCar && (
        <BookingModal car={selectedCar} isOpen={isBookingModalOpen} onClose={closeBookingModal} />
      )}

      <StructuredData type="Fleet" data={{ numberOfItems: filteredCars.length }} countryTarget="MA" language="en" />
      <BreadcrumbStructuredData items={[{ label: 'Our Fleet', href: '/en/fleet' }]} />
    </div>
  )
}
