...
import { X, CheckCircle, Mail, AlertTriangle } from 'lucide-react'
...
export default function BookingConfirmationPopup({ isOpen, onClose, bookingId, totalPrice }: BookingConfirmationPopupProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[60] flex items-center justify-center p-2 sm:p-4">
      <div className={`bg-white rounded-2xl sm:rounded-3xl shadow-2xl max-w-sm sm:max-w-md w-full mx-2 sm:mx-4 max-h-[90vh] overflow-y-auto transform transition-all duration-300 ${isVisible ? 'scale-100 opacity-100' : 'scale-95 opacity-0'}`}>
        <div className="relative bg-gradient-to-r from-green-500 to-emerald-600 rounded-t-2xl sm:rounded-t-3xl p-4 sm:p-6 text-center">
          <button onClick={handleClose} className="absolute top-3 right-3 sm:top-4 sm:right-4 text-white/80 hover:text-white transition-colors">
            <X className="h-5 w-5 sm:h-6 sm:w-6" />
          </button>
          <div className="flex justify-center mb-3 sm:mb-4"><div className="bg-white/20 rounded-full p-2 sm:p-3"><CheckCircle className="h-8 w-8 sm:h-10 sm:w-10 text-white" /></div></div>
          <h2 className="text-lg sm:text-xl font-bold text-white mb-1 sm:mb-2">Reserva pendiente de confirmación</h2>
          <p className="text-green-100 text-sm sm:text-base">Número de reserva: #{bookingId}</p>
        </div>
