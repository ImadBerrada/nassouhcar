...
export default function FleetPage() {
  ...
  const fetchCars = useCallback(async () => {
    try {
      ...
      const message = err instanceof Error ? err.message : 'Error al obtener coches';
      setError(message)
    } finally {
      setLoading(false)
    }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 to-slate-100">
        <div className="text-center">
          ...
          <p className="text-slate-600 text-lg">Cargando nuestra flota económica en Tánger...</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100">
      ...
      <div className="relative bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 py-16 overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <h1 className="text-6xl md:text-7xl font-black text-white mb-6 leading-tight tracking-tight">
            Alquiler Coche Tánger - Flota Económica
          </h1>
          <p className="text-xl md:text-2xl text-gray-300 max-w-4xl mx-auto leading-relaxed">
            Descubra nuestra flota de vehículos económicos para el alquiler de coches en Tánger. Desde coches asequibles hasta vehículos familiares, encuentre el vehículo perfecto para su estancia en Marruecos con Nassoh Car.
          </p>
          <div className="mt-8 flex flex-wrap justify-center gap-4 text-amber-400">
            <span className="bg-white/10 px-4 py-2 rounded-full">✓ Aeropuerto Tánger Ibn Battouta</span>
            <span className="bg-white/10 px-4 py-2 rounded-full">✓ Puerto Tanger Med</span>
            <span className="bg-white/10 px-4 py-2 rounded-full">✓ Centro de Tánger</span>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
        <Breadcrumb 
          items={[
            { label: 'Nuestra Flota', href: '/es/flota' }
          ]}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="mb-8">
          <h2 className="text-2xl font-bold text-slate-900 mb-2">
            Vehículos Disponibles en Tánger ({filteredCars.length})
          </h2>
          <p className="text-slate-600">
            {searchQuery ? `Resultados para "${searchQuery}"` : 'Todos los vehículos disponibles para alquiler en Tánger'}
          </p>
        </div>
        ...
        {filteredCars.length === 0 && (
          <div className="text-center py-16">
            ...
            <h3 className="text-2xl font-bold text-slate-900 mb-4">Ningún vehículo encontrado</h3>
            <p className="text-slate-600 mb-6">
              {searchQuery ? 'Intente ajustar sus términos de búsqueda' : 'Ningún vehículo está disponible actualmente'}
            </p>
            {searchQuery && (
              <button
                ...
                <div className="p-6 flex flex-col flex-grow">
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-6">
                    <div className="flex items-center">
                      ...
                      <div>
                        <span className="text-xs text-gray-500 block">Plazas</span>
                  ...
                  <div className="flex items-center justify-between pt-6 border-t border-gray-100 mt-auto">
                    <div className="flex flex-col">
                      <span className="text-3xl font-bold text-gray-900">desde €{car.pricePerDay.toFixed(2)}</span>
                      <span className="text-sm text-gray-500 font-medium">por día</span>
                    </div>
                    <button
                    ...
                    >
                      {(car.availableQuantity || car.quantity) > 0 ? 'Reservar Ahora' : 'No Disponible'}
                    </button>
                  </div>
                </div>
              </div>
