...
export default function LoginPageES() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Inicio de sesión fallido. Inténtelo de nuevo.')
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-8">
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          <div className="bg-white p-8 rounded-xl shadow-lg space-y-6">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="text-sm"><Link href="/es/restablecer-contrasena" className="font-medium text-blue-600 hover:text-blue-500">¿Olvidaste tu contraseña?</Link></div>
            </div>
        ...
        <div className="text-center"><p className="text-sm text-gray-600">¿No tienes cuenta? <Link href="/es/registro" className="font-medium text-blue-600 hover:text-blue-500">Regístrate aquí</Link></p></div>
      </div>
    </div>
  )
}
