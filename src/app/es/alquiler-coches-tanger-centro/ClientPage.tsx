...
import { Car as CarIcon, Users, Settings, Fuel } from 'lucide-react'
...
export default function CarRentalTangierCityCenterClient() {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100">
      <div className="relative bg-gradient-to-r from-orange-900 via-orange-800 to-orange-900 py-20 overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <h1 className="text-4xl md:text-6xl font-bold text-white mb-6 leading-tight">Alquiler de Coches Tánger Centro</h1>
          <p className="text-xl text-orange-100 mb-8 max-w-3xl mx-auto leading-relaxed">Recogida en el corazón de Tánger, cerca de la medina y atracciones principales. Horarios flexibles y una flota moderna.</p>
        </div>
      </div>
      <div className="py-16 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12"><h2 className="text-3xl font-bold text-gray-900 mb-4">Vehículos Disponibles en el Centro</h2><p className="text-gray-600">Elija entre opciones compactas, económicas y SUV para conducir en la ciudad.</p></div>
          {loading ? (
            <div className="text-center py-12"><div className="animate-spin rounded-full h-12 w-12 border-b-2 border-orange-500 mx-auto mb-4"></div><p className="text-gray-600">Cargando vehículos disponibles...</p></div>
          ) : (
              ...
              {cars.slice(0, 6).map((car) => (
                <div key={car.id} className="bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-xl transition-shadow">
                  ...
                  <div className="p-6">
                    ...
                    <div className="flex items-center justify-between"><div><span className="text-2xl font-bold text-gray-900">desde €{car.pricePerDay.toFixed(2)}</span><span className="text-gray-500 text-sm">/día</span></div><button onClick={() => handleBookNow(car)} className="bg-orange-500 text-white px-6 py-2 rounded-lg hover:bg-orange-600 transition-colors font-semibold">Reservar Ahora</button></div>
                  </div>
                </div>
              ))}
            </div>
